...
/*************** Millis timer *******************/
  ...
  // serial mapping changer interval
...
void setup() {
  ...
  if (EEPROM.read(13) != 0) { // add to 0 for updates to EEPROM management/preset
    EEPROM.write(0, 4);
    EEPROM.write(1, 200);
    EEPROM.write(2, 200);
    EEPROM.write(3, 150);
  ...
  bVal       = EEPROM.read(7);
  mapping[0] = EEPROM.read(40);
  mapping[1] = EEPROM.read(41);
}
...
void loop() {
  ...
  // Runs all LED code every 4 ms
  if ((currentMillis - loopCheck) > 4) {
    /* Mode selection */
    switch(buttonMode) {
      case 0:
        cycle();
    ...
    loopCheck = currentMillis;
  }

  // if (Serial) will greatly reduce the speed of the code,
  // so it will instead be run every 100 ms
  if ((currentMillis - previousMillis) > 100) {
    if (Serial && set == 0) remapSerial();
...
void sideButton() {
  ...
  if (digitalRead(b3)){
    ...
    if (bV == 4) { // Write brightness to EEPROM
      EEPROM.write(7, bVal);
    }
...
/* Blanking function when changing modes */
...
void remapSerial() {
  ...
  for (j = 0; j < NUMKEYS; j++) {
    Serial.print("Please input a-z value for button ");
    Serial.print(j+1);
    Serial.print(": ");

    // Wait for serial input vis serial monitor
    while(!Serial.available()){}
    char b1 = Serial.read();
    Serial.println(b1);
    EEPROM.write(40+j, int(b1));
    mapping[j] = int(b1);
  }

  Serial.print("Buttons set! New buttons are: ( ");
  for (j = 0; j < NUMKEYS; j++) {
    Serial.print(char(mapping[j]));
    Serial.print(" ");
  }
  Serial.println(")");
  Serial.println("To set again, please unplug the keypad and plug it back in.");
